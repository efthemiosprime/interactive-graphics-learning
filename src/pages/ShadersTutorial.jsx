...
export default function ShadersTutorial() {
  ...
  const [activeTutorial, setActiveTutorial] = useState('vite'); // 'vite' or 'threejs'
...
// ... (full code as shown above)`
  ...
  const threejsSteps = [
    {
      title: 'Step 1: Set Up Vite Project with Three.js',
      description: 'Create a new Vite project and install Three.js',
      code: `# Create a new Vite project
npm create vite@latest shader-threejs -- --template vanilla
...
# Install Three.js
npm install three`,
      explanation: 'Three.js is a powerful 3D library that simplifies WebGL. We use Vite for fast development and Three.js for easier shader management.',
      files: [
        {
          name: 'package.json',
          content: `{
  ...
  "devDependencies": {
    ...
    {
      title: 'Step 2: Create HTML Structure',
      description: 'Set up the basic HTML file with a container for Three.js scene',
      code: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Shader Visualization with Three.js</title>
  <style>
    body {
      margin: 0;
      padding: 0;
...
<body>
  ...
  <script type="module" src="/main.js"></script>
      ...
      explanation: 'Three.js renders into a container div. We use absolute positioning for controls overlay.',
      files: [
        {
          name: 'index.html',
          content: `<!DOCTYPE html>
...
<body>
  ...
  <script type="module" src="/main.js"></script>
    ...
    {
      title: 'Step 3: Initialize Three.js Scene',
      description: 'Create scene, camera, and renderer',
      code: `// main.js
...
console.log('Three.js scene initialized!');`,
      explanation: 'Three.js abstracts WebGL setup. Scene holds objects, camera defines view, renderer handles drawing. We set up responsive resizing.',
      files: [
        {
          name: 'main.js',
...
window.addEventListener('resize', () => {
  ...
  renderer.setSize(window.innerWidth, window.innerHeight);
    ...
    {
      title: 'Step 4: Create Custom Shader Material',
      description: 'Define vertex and fragment shaders for custom material',
      code: `// shaders/customShader.js
...
void main() {
  ...
  // Mix with uniform color
  color = mix(color, u_color, 0.3);
  
  gl_FragColor = vec4(color, 1.0);
}
\`;`,
      explanation: 'Three.js uses GLSL shaders similar to WebGL. Vertex shader passes UV coordinates, fragment shader creates the visual effect. Uniforms are passed from JavaScript.',
      files: [
        {
          name: 'shaders/customShader.js',
...
\`;`
    ...
    {
      title: 'Step 5: Create Geometry and Apply Shader',
      description: 'Create a plane geometry and apply custom shader material',
      code: `// main.js (continued)
import { vertexShader, fragmentShader } from './shaders/customShader.js';
...
const material = new THREE.ShaderMaterial({
  ...
  uniforms: {
    ...
    u_color: { value: new THREE.Vector3(0.5, 0.5, 1.0) }
  }
});
...
renderer.render(scene, camera);`,
      explanation: 'ShaderMaterial lets us use custom GLSL shaders. Uniforms are passed as an object. The plane geometry creates a full-screen quad when positioned correctly.',
      files: [
        {
          name: 'main.js (continued)',
          content: `import { vertexShader, fragmentShader } from './shaders/customShader.js';
...
renderer.render(scene, camera);`
    ...
    {
      title: 'Step 6: Create Animation Loop',
      description: 'Update uniforms and render each frame',
      code: `// Animation loop
let startTime = Date.now();
...
animate();`,
      explanation: 'Three.js animation loop updates uniforms each frame. requestAnimationFrame provides smooth 60fps rendering. Uniforms are accessed via material.uniforms.',
      files: [
        {
          name: 'main.js (animation loop)',
          content: `let startTime = Date.now();
...
animate();`
    ...
    {
      title: 'Step 7: Add Interactive Controls',
      description: 'Create UI controls to modify shader parameters',
      code: `// Add dat.GUI for controls (optional but recommended)
// npm install dat.gui
...
gui.addColor(params, 'color').onChange((color) => {
  material.uniforms.u_color.value.set(
    color.r / 255,
    color.g / 255,
    color.b / 255
  );
...
// Or create custom HTML controls
function createControl(label, min, max, value, callback) {
  const container = document.createElement('div');
  container.className = 'control-item';
  ...
  return container;
      ...
      explanation: 'Three.js works well with dat.GUI for quick controls, or you can create custom HTML controls. Update material.uniforms to change shader parameters in real-time.',
      files: [
        {
          name: 'controls.js',
          content: `export function createControl(label, min, max, value, callback) {
  ...
  return container;
    ...
    {
      title: 'Step 8: Add More Complex Shader Effects',
      description: 'Create advanced shader effects with noise and patterns',
      code: `// Advanced fragment shader with noise
...
void main() {
  ...
  // Animated noise
  float n = noise(uv * 5.0 + u_time * u_speed);
...
\`;`,
      explanation: 'Three.js makes it easy to create complex shader effects. You can add noise functions, patterns, and advanced math operations. Uniforms allow real-time parameter control.',
      files: [
        {
          name: 'shaders/advancedShader.js',
...
\`;`
    ...
    {
      title: 'Step 9: Load and Apply Textures',
      description: 'Load textures and use them in shaders',
      code: `// Load texture
const textureLoader = new THREE.TextureLoader();

textureLoader.load('/texture.jpg', (texture) => {
  // Update shader material to use texture
  ...
  const fragmentShaderWithTexture = \`
    ...
    void main() {
      ...
      color += sin(u_time) * 0.1;
      
      gl_FragColor = vec4(color, 1.0);
    }
  \`;
  
  material.fragmentShader = fragmentShaderWithTexture;
  material.needsUpdate = true;
});`,
      explanation: 'Three.js TextureLoader simplifies loading images. Update material uniforms and set needsUpdate=true when changing shader code. Textures can be sampled in fragment shaders.',
      files: [
        {
          name: 'main.js (texture loading)',
          content: `const textureLoader = new THREE.TextureLoader();
...
textureLoader.load('/texture.jpg', (texture) => {
  ...
  material.needsUpdate = true;
    ...
    {
      title: 'Step 10: Complete Example - Animated Shader',
      description: 'Put it all together with a complete working example',
      code: `// Complete main.js
import * as THREE from 'three';
import { vertexShader, fragmentShader } from './shaders/customShader.js';
...
animate();`,
      explanation: 'This complete example shows all Three.js concepts together. The shader creates an animated gradient effect. Three.js handles WebGL complexity, letting you focus on shader code.',
      files: [
        {
          name: 'main.js (complete)',
          content: `import * as THREE from 'three';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-purple-900">
              Complete Tutorial: Creating Shader Visualization
            </h1>
          </div>

          {/* Tutorial Type Tabs */}
          <div className="flex gap-2 mb-6 border-b-2 border-purple-200 pb-4">
            <button
              onClick={() => {
                setActiveTutorial('vite');
                setExpandedSteps({});
              }}
            ...
            <button
              onClick={() => {
                setActiveTutorial('threejs');
                setExpandedSteps({});
              }}
              className={`px-6 py-3 rounded-lg font-semibold transition-all ${
                activeTutorial === 'threejs'
                  ? 'bg-purple-600 text-white shadow-lg'
                  : 'bg-purple-100 text-purple-700 hover:bg-purple-200'
              }`}
            >
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
          <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 rounded">
            ...
            <ul className="list-disc list-inside text-blue-800 space-y-1">
              {activeTutorial === 'vite' ? (
                ...
                <>
                  <li>Setting up a Vite project with Three.js</li>
                  <li>Creating Three.js scene, camera, and renderer</li>
                  <li>Writing custom shader materials</li>
                  <li>Using ShaderMaterial for custom effects</li>
                  <li>Creating geometry and meshes</li>
                  <li>Updating uniforms for animations</li>
                  <li>Creating animation loops with Three.js</li>
                  <li>Adding interactive controls and textures</li>
                </>
              )}
            </ul>
          </div>
          ...
          <div className="space-y-4">
            {(activeTutorial === 'vite' ? steps : threejsSteps).map((step, index) => (
                ...
                <ul className="list-disc list-inside text-green-800 space-y-1">
                  {activeTutorial === 'vite' ? (
                    <>
                      ...
                      <li>Consider learning Three.js for easier 3D shader development</li>
                    </>
                  ) : (
                    <>
                      <li>Experiment with different shader effects and patterns</li>
                      <li>Add more uniforms for interactive controls</li>
                      <li>Try using Three.js built-in materials and combining with custom shaders</li>
                      <li>Explore post-processing effects with THREE.EffectComposer</li>
                      <li>Learn about Three.js geometry and how to apply shaders to different shapes</li>
                      <li>Check out Three.js examples and documentation for advanced techniques</li>
                      <li>Try creating 3D objects with custom shader materials</li>
                    </>
                  )}
                </ul>
              </div>
