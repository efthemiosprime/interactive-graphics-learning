...
import { getAnimationTheory } from '../../utils/animationInterpolationTheory';
import { getAnimationFormulas } from '../../utils/animationInterpolationFormulas';
import { getAnimationMathConcepts } from '../../utils/animationInterpolationMathConcepts';
import { getAnimationStepByStep } from '../../utils/animationInterpolationStepByStep';
import AnimationInterpolationCodeDisplay from './AnimationInterpolationCodeDisplay';

const AnimationInterpolationEducationalPanels = ({
  interpolationType,
  linearStart,
  linearEnd,
  easingType,
  ...
  const theory = getAnimationTheory(interpolationType);
  const mathConcepts = getAnimationMathConcepts(interpolationType);
  
  // Get formulas with current parameter values
  let formulaParams = null;
  if (interpolationType === 'linear') {
    formulaParams = { start: linearStart, end: linearEnd };
  } else if (interpolationType === 'easing') {
    formulaParams = { type: easingType, power: easingPower };
  } else if (interpolationType === 'bezierEasing') {
    formulaParams = { points: bezierEasingPoints };
  } else if (interpolationType === 'slerp') {
    formulaParams = { start: slerpStart, end: slerpEnd };
  } else if (interpolationType === 'keyframes') {
    formulaParams = { keyframes, interpolation: keyframeInterpolation };
  } else if (interpolationType === 'path') {
    formulaParams = { points: pathPoints, pathType };
  }
  
  const formulas = getAnimationFormulas(interpolationType, formulaParams);
  
  // Get step-by-step explanation
  const stepByStep = getAnimationStepByStep(interpolationType, formulaParams);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Code Display */}
      <AnimationInterpolationCodeDisplay interpolationType={interpolationType} />
    </div>
  );
};

export default AnimationInterpolationEducationalPanels;
