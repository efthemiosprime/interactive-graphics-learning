...
import { getEngine2DTheory } from '../../utils/engine2DTheory';
import { getEngine2DFormulas } from '../../utils/engine2DFormulas';
import { getEngine2DMathConcepts } from '../../utils/engine2DMathConcepts';
import { getEngine2DStepByStep } from '../../utils/engine2DStepByStep';
import { getEngine2DPrerequisites } from '../../utils/engine2DPrerequisites';
import { getEngine2DCommonPitfalls } from '../../utils/engine2DCommonPitfalls';
import { getEngine2DGlossary } from '../../utils/engine2DGlossary';
import { getEngine2DBestPractices } from '../../utils/engine2DBestPractices';
import { getEngine2DDebuggingTips } from '../../utils/engine2DDebuggingTips';

const Engine2DEducationalPanels = ({
  currentModule,
  currentStep
}) => {
  const theory = getEngine2DTheory(currentModule, currentStep);
  const mathConcepts = getEngine2DMathConcepts(currentModule, currentStep);
  const formulas = getEngine2DFormulas(currentModule, currentStep);
  const stepByStep = getEngine2DStepByStep(currentModule, currentStep);
  const prerequisites = getEngine2DPrerequisites(currentModule, currentStep);
  const pitfalls = getEngine2DCommonPitfalls(currentModule, currentStep);
  const glossary = getEngine2DGlossary(currentModule, currentStep);
  const bestPractices = getEngine2DBestPractices(currentModule, currentStep);
  const debuggingTips = getEngine2DDebuggingTips(currentModule, currentStep);
    ...
    <div className="space-y-6">
      ...
      {theory && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-bold text-purple-900 mb-4">{theory.title}</h3>
      ...
      {prerequisites && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-bold text-purple-900 mb-4">{prerequisites.title}</h3>
      ...
      {pitfalls && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-bold text-purple-900 mb-4">{pitfalls.title}</h3>
          <div className="space-y-4">
      ...
      {glossary && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-bold text-purple-900 mb-4">{glossary.title}</h3>
          <div className="space-y-3">
      ...
      {bestPractices && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-bold text-purple-900 mb-4">{bestPractices.title}</h3>
          <div className="space-y-3">
      ...
      {debuggingTips && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-bold text-purple-900 mb-4">{debuggingTips.title}</h3>
          <div className="space-y-3">
...
export default Engine2DEducationalPanels;
